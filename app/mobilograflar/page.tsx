...
function ProjectCard({ project, mobId, selectedYear, selectedMonth, onReassign }: any) {
      ...
      <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-5 border border-purple-200 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            <p className="text-5xl font-bold text-gray-900">{stats.montaj}</p>
            <p className="text-sm text-gray-600 mt-1">/ {project.monthly_target} post</p>
          </div>
        </div>
        ...
        <div className="mt-2 text-right">
          <span className="text-2xl font-bold text-purple-600">{stats.montajProgress}%</span>
        </div>
      </div>
  ...
  const loadAvailableYears = async () => {
    try {
      ...
      videos.forEach(video => {
        if (video.work_date) {
          const year = new Date(video.work_date).getFullYear()
          years.add(year)
        }
      })
